...
def map_operation(operation: Operation) -> Instruction:
    ...
    if isinstance(operation, ClassicallyControlledOperation):
        if len(operation._conditions) > 1:
            raise OperationNotSupportedError('Classically controlled prx gates can currently only have one condition.')
        ...
        raise OperationNotSupportedError(
            f'Classical control on the {operation._sub_operation.gate} gate is not natively supported'
        )
    ...
    if cc_prx_support:
        ...
        for inst in instructions:
            if inst.name == 'measure':
                if inst.args['key'] in mkey_to_measurement.keys():
